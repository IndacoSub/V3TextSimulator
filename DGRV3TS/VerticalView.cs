...
namespace DGRV3TS
{
	public partial class VerticalView : Form
	{
		...
		public void InitSummaries(List<string> original, List<string> translated, List<string> speakers)
		{
			...
			for (int i = 0; i < 100; i++)
			{
				panel1.VerticalScroll.Value = GetScrollValue();
				panel1.AutoScrollPosition = new Point(panel1.AutoScrollPosition.X, GetScrollValue());
			}
			//this.Invalidate();
		}
		...
		public void UpdateSummaries(List<string> original, List<string> translated, int index)
		{
			...
			for (int i = 0; i < original.Count; i++)
			{
				...
				var _panel = this.panel1;
				if (_panel == null)
				{
					break;
				}

				Control.ControlCollection _controls = _panel.Controls;
				if (_controls == null)
				{
					break;
				}
				...
				if (_controls.Count <= valid_index)
				{
					InputManager.Print("Invalid control index: " + valid_index + ", arg index: " + index);
					break;
				}

				var single_control = _controls[valid_index];
		...
		{
			SetScrollValue(panel1.VerticalScroll.Value);
